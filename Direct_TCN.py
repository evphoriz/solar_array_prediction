...
def data_build(X, y, timesteps, horizon, stride):
    ...
    for i in range(0, end, stride):
        ...
        x_data.append(seq_x)
        y_data.append(seq_y)
    X_out = np.stack(x_data, axis=0)  # (num_samples, timesteps, features)
    y_out = np.stack(y_data, axis=0)  # (num_samples, horizon)
    return X_out, y_out
...
def load_downsample(rout_files, mane_files, num_sk=3036):
    ...
    for file in rout_files:
        ...
        if len(label_lg_indices) > p:
            out_lg_indices = last_label_os_index - len(label_es_indices) - len(label_is_indices) - len(label_os_indices)  #出影期后，光照0的索引
...
class TemporalBlock(nn.Module):
    ...
    def forward(self, x):
        y = self.dropout1(self.relu1(self.chomp1(self.conv1(x))))
        y = self.dropout2(self.relu2(self.chomp2(self.conv2(y))))
        res = x if self.downsample is None else self.downsample(x)
        return self.relu(y + res)


class DirectTCN(nn.Module):
    def __init__(self, num_inputs, num_channels, horizon,
                 kernel_size=3, dropout=0.1):
        super(DirectTCN, self).__init__()
        layers = []
...
def estimate_flops_tcn(T: int, H: int, m: int, h: int, L: int, kernel_size: int) -> int:

    flops_enc_first = 2 * (m * h + h * h) * T * kernel_size
    flops_enc_rest  = 0
    for _ in range(L - 1):
        flops_enc_rest += 2 * (h * h + h * h) * T * kernel_size

    flops_enc = flops_enc_first + flops_enc_rest

    flops_fc = 2 * h * H

    total_flops = flops_enc + flops_fc
...
if __name__ == "__main__":
    ...
    horizon = 10
    stride = 10
    tcn_channels = [256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256]
    kernel_size = 5
    dropout = 0.1
    learning_rate = 0.001
    ...
    # 实例化模型、损失函数以及优化器
    model = DirectTCN(input_dim, tcn_channels, horizon, kernel_size, dropout).to(device)
    ...
    for run_id, seed in enumerate(seeds, 1):
        ...
        X_train_seq, y_train_seq = data_build(std_train_data[:, :-1], std_train_data[:, -1], time_steps, horizon,
                                              stride)
        ...
        flops_tcn = estimate_flops_tcn(T=time_steps, H=horizon, m=input_dim, h=hidden_dim, L=layers_num, kernel_size=kernel_size)

        print("可训练总参数量:", count_trainable_parameters(model))
        print(f"总FLOPs：{flops_tcn:,} FLOPs")
