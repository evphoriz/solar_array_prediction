...
class DirRecWrapper(nn.Module):
    def __init__(self, backbone: nn.Module, head_direct: nn.Module, head_step: nn.Module, horizon: int, block: int):
        super().__init__()
    ...
    def forward(self, x):
        z_seq = self.backbone(x)
        z = z_seq[:, -1, :] if z_seq.dim() == 3 else z_seq
        B, H, Bk = x.size(0), self.horizon, min(self.block, self.horizon)
        y = x.new_zeros(B, H)
        if Bk > 0:
            y[:, :Bk] = self.head_direct(z).view(B, -1)[:, :Bk]
        prev = y[:, Bk-1].unsqueeze(1) if Bk > 0 else self.head_step(torch.cat([z, x.new_zeros(B,1)], dim=1))
        for t in range(Bk, H):
            y_t = self.head_step(torch.cat([z, prev], dim=1))
            y[:, t:t+1] = y_t
            prev = y_t
        return y
...
def estimate_flops_dirrec(input_dim: int, hidden_dim: int, num_layers, horizon: int, block: int, T: int, bidirectional: bool) -> int:

    if bidirectional:
        hidden_dim *= 2


    flops_per_step_lstm = 2 * (input_dim * hidden_dim + hidden_dim * hidden_dim)
    flops_per_layer_lstm = flops_per_step_lstm * T
    flops_lstm = flops_per_layer_lstm * num_layers

    flops_head_direct = 2 * hidden_dim * 1

    flops_fc1 = 2 * (hidden_dim + 1) * (hidden_dim * 2)
    flops_fc2 = 2 * hidden_dim * (2 * hidden_dim)
    flops_fc3 = 2 * (2 * hidden_dim) * hidden_dim
    flops_fc4 = 2 * hidden_dim * 1
    flops_head_step = flops_fc1 + flops_fc2 + flops_fc3 + flops_fc4

    flops_per_horizon = flops_lstm + flops_head_direct
    flops_total = flops_per_horizon * block

    flops_recursion = flops_head_step * (horizon - block)

    total_flops = flops_total + flops_recursion
    ...
    with torch.no_grad():
        for inputs, targets in test_loader:
            ...
            if return_latency:
                ...
                lat_total_ms += dt
                lat_steps += (inputs.size(0) * preds.size(1))  # 样本×预测步
...
if __name__ == "__main__":
    ...
    # 实例化模型、损失函数以及优化器
    backbone = Backbone(input_dim, hidden_dim, layers_num).to(device)
    head_direct  = HeadDirect(hidden_dim)
    head_step = HeadStep(hidden_dim, dropout)
    model = DirRecWrapper(backbone, head_direct, head_step,horizon, block).to(device)
    ...
    for run_id, seed in enumerate(seeds, 1):
        ...
        flops_dirrec = estimate_flops_dirrec(input_dim, hidden_dim, layers_num, horizon, block, time_steps, bidirectional)

        print("可训练总参数量:", count_trainable_parameters(model))
        print(f"总FLOPs：{flops_dirrec:,} FLOPs")
