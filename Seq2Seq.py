...
class Seq2Seq(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, num_layers, dropout=0.2):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=1.0):
        ...
        outputs = torch.zeros(batch_size, trg_len, output_dim, device=src.device, dtype=src.dtype)
        ...
        for t in range(trg_len):
            ...
            if t + 1 < trg_len:
                use_tf = (torch.rand(1, device=src.device) < teacher_forcing_ratio)
                input = trg[:, t, :].unsqueeze(1) if use_tf else out
...
def estimate_flops_seq2seq(input_dim: int, output_dim: int, hidden_dim: int, num_layers: int,
                           src_len: int, trg_len: int) -> int:

    flops_per_step_lstm = 2 * (input_dim * hidden_dim + hidden_dim * hidden_dim)
    flops_per_layer_lstm_encoder = flops_per_step_lstm * src_len
    flops_lstm_encoder = flops_per_layer_lstm_encoder * num_layers

    flops_per_step_lstm_decoder = 2 * (output_dim * hidden_dim + hidden_dim * hidden_dim)
    flops_per_layer_lstm_decoder = flops_per_step_lstm_decoder * trg_len
    flops_lstm_decoder = flops_per_layer_lstm_decoder * num_layers


    flops_fc = 2 * hidden_dim * output_dim

    flops_per_time_step = flops_lstm_decoder + flops_fc

    total_flops = flops_lstm_encoder + (flops_per_time_step * trg_len)
...
def train_model(train_loader, model, criterion, optimizer, device, epochs):
    ...
    for epoch in range(epochs):
        ...
        for inputs, targets in train_loader:
            ...
            outputs = model(inputs, targets, teacher_forcing_ratio=1.0)
...
if __name__ == "__main__":
    ...
    # 实例化模型、损失函数以及优化器
    model = Seq2Seq(input_dim, output_dim, hidden_dim, layers_num, dropout).to(device)
    ...
    for run_id, seed in enumerate(seeds, 1):
        ...
        if device.type == 'cuda':
            ...
            probe_reg_infer = measure_vram_peak_generic(
                model, xb_te, yb_te, train=False, use_amp=USE_AMP, classify_loss_fn=None)
        ...
        flops_seq = estimate_flops_seq2seq(input_dim, output_dim, hidden_dim, layers_num, time_steps, horizon)

        print("可训练总参数量:", count_trainable_parameters(model))
        print(f"总FLOPs：{flops_seq:,} FLOPs")
