...
class DirectLSTM(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, horizon: int, num_layers: int = 3,
        ...
        self.fc2 = nn.Linear(2 * hidden_dim, horizon)
...
def estimate_flops_lstm(T: int, H: int, input_dim: int, hidden_dim: int, num_layers: int, bidirectional: bool) -> int:

    if bidirectional:
        hidden_dim *= 2

    flops_per_step = 2 * (input_dim * hidden_dim + hidden_dim * hidden_dim)

    flops_per_layer = flops_per_step * T

    flops_lstm = flops_per_layer * num_layers

    flops_fc = 2 * hidden_dim * H

    total_flops = flops_lstm + flops_fc
...
if __name__ == "__main__":
    ...
    # 实例化模型、损失函数以及优化器
    model = DirectLSTM(input_dim, hidden_dim, horizon, layers_num, bidirectional, dropout).to(device)
    ...
    for run_id, seed in enumerate(seeds, 1):
        ...
        flops_lstm = estimate_flops_lstm(time_steps, horizon, input_dim, hidden_dim, layers_num, bidirectional)

        print("可训练总参数量:", count_trainable_parameters(model))
        print(f"总FLOPs：{flops_lstm:,} FLOPs")
